...
class boss_patchwerk_progression : public CreatureScript
{
public:
    boss_patchwerk_progression() : CreatureScript("boss_patchwerk") { }

    CreatureAI* GetAI(Creature* pCreature) const override
    {
        return GetNaxxramasAI<boss_patchwerk_progressionAI>(pCreature);
    }

    struct boss_patchwerk_progressionAI : public BossAI
    {
        explicit boss_patchwerk_progressionAI(Creature* c) : BossAI(c, BOSS_PATCHWERK)
        {
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            switch (events.ExecuteEvent())
            {
                ...
                case EVENT_BERSERK:
                    ...
                    if (sConfigMgr->GetOption<uint32>("Progression.Patch", PATCH_ASSAULT_ON_THE_RUBY_SANCTUM) >= PATCH_DRUMS_OF_WAR)
...
{
    new boss_patchwerk_progression();
}
