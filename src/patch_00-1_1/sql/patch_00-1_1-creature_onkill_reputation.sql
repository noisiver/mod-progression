UPDATE `creature_onkill_reputation` SET `MaxStanding1`=4 WHERE `creature_id` IN (
...
UPDATE `creature_onkill_reputation` SET `RewOnKillRepValue1`=15 WHERE `creature_id` IN (
    1847, -- Foulmane
...
UPDATE `creature_onkill_reputation` SET `MaxStanding2`=4 WHERE `creature_id` IN (
    4638, -- Magram Scout
    4639, -- Magram Outrunner
    4640, -- Magram Wrangler
    4641, -- Magram Windchaser
...
UPDATE `creature_onkill_reputation` SET `MaxStanding2`=5 WHERE `creature_id` IN (
    4624 -- Booty Bay Bruiser
);

UPDATE `creature_onkill_reputation` SET `RewOnKillRepValue2`=0 WHERE `creature_id` IN (
...
UPDATE `creature_onkill_reputation` SET `RewOnKillRepValue2`=150 WHERE `creature_id` IN (
    15340, -- Moam
    15348, -- Kurinnaxx
