UPDATE `item_template` SET `Quality` = 3 WHERE `entry` IN (7513, 7514, 9517, 11263);
UPDATE `item_template` SET `RequiredLevel` = 60 WHERE `entry` = 17082;
UPDATE `item_template` SET `dmg_min1` = 36 WHERE `entry` = 7513;
UPDATE `item_template` SET `dmg_min1` = 39 WHERE `entry` = 11263;
UPDATE `item_template` SET `dmg_min1` = 41 WHERE `entry` = 7514;
UPDATE `item_template` SET `dmg_min1` = 85 WHERE `entry` = 9517;
UPDATE `item_template` SET `dmg_max1` = 68 WHERE `entry` = 7513;
UPDATE `item_template` SET `dmg_max1` = 73 WHERE `entry` = 11263;
UPDATE `item_template` SET `dmg_max1` = 78 WHERE `entry` = 7514;
UPDATE `item_template` SET `dmg_max1` = 129 WHERE `entry` = 9517;
UPDATE `item_template` SET `stat_type1` = 5 WHERE `entry` IN (7513, 7514, 11263);
UPDATE `item_template` SET `stat_type3` = 0 WHERE `entry` = 16803;
UPDATE `item_template` SET `stat_value1` = 3 WHERE `entry` IN (7513, 7514, 11263);
UPDATE `item_template` SET `stat_value1` = 8 WHERE `entry` = 9517;
UPDATE `item_template` SET `stat_value1` = 13 WHERE `entry` = 18208;
UPDATE `item_template` SET `stat_value2` = 11 WHERE `entry` = 18208;
UPDATE `item_template` SET `stat_value3` = 8 WHERE `entry` = 18208;
UPDATE `item_template` SET `spellid_1` = 7688 WHERE `entry` = 7513;
UPDATE `item_template` SET `spellid_1` = 7702 WHERE `entry` = 7514;
UPDATE `item_template` SET `spellid_1` = 9317 WHERE `entry` = 18208;
UPDATE `item_template` SET `spellid_1` = 13595 WHERE `entry` = 11263;
UPDATE `item_template` SET `spellid_1` = 15714 WHERE `entry` = 9517;
UPDATE `item_template` SET `spellid_2` = 0 WHERE `entry` = 9517;
UPDATE `item_template` SET `spellid_3` = 0 WHERE `entry` = 9517;
UPDATE `item_template` SET `spelltrigger_3` = 0 WHERE `entry` = 9517;
UPDATE `item_template` SET `spellcooldown_1` = 600000 WHERE `entry` = 13937;
UPDATE `item_template` SET `spellcategorycooldown_1` = 60000 WHERE `entry` = 13937;
UPDATE `item_template` SET `StatsCount` = 1 WHERE `entry` IN (7513, 7514, 11263);
UPDATE `item_template` SET `StatsCount` = 2 WHERE `entry` = 16803;
