UPDATE `creature` SET `position_x` = -8439.61, `position_y` = 334.384, `position_z` = 122.58, `orientation` = 2.2081 WHERE `id1` = 1747;
UPDATE `creature` SET `phaseMask` = 1 WHERE `guid` IN (97718, 97736, 128078, 128079, 203492, 203493, 203494, 203495);
UPDATE `creature` SET `phaseMask` = 1 WHERE `id1` IN (26537, 26539, 26548, 28092, 28601, 28602, 29016, 29019, 29088, 29141, 29142, 29143, 29144, 29152, 29154, 29287, 29288, 29289, 29290, 29291, 29292, 29293, 29294, 29295, 29296, 29297, 29298, 29299, 29300, 29346, 29611, 29712, 29725, 29728, 30706, 30709, 30710, 30711, 30713, 30715, 30716, 30717, 30721, 30722, 30723, 30724, 30727, 30729, 30730, 30731, 30732, 30733, 30734, 30735, 31144, 31146, 32666, 32667, 33638, 33679, 34043, 34081, 35596, 35597, 35598, 35599, 35600, 35601, 35602, 35603, 35611, 35612);
UPDATE `creature` SET `spawntimesecs` = 3600 WHERE `guid` IN (128070, 128071, 128073, 128074, 128077);
UPDATE `creature` SET `id1` = 16018, `spawntimesecs` = 3600 WHERE `guid` IN (128075, 128084);
UPDATE `creature` SET `position_x` = 3159.19, `position_y` = -3270.79, `position_z` = 294.84, `orientation` = 3.97935 WHERE `guid` = 128070;
UPDATE `creature` SET `position_x` = 3168.32, `position_y` = -3280.82, `position_z` = 294.92, `orientation` = 4.06662 WHERE `guid` = 128071;
UPDATE `creature` SET `id1` = 16018, `spawntimesecs` = 3600 WHERE `guid` = 128082;
UPDATE `creature` SET `id1` = 16029, `spawntimesecs` = 3600 WHERE `guid` = 97747;
UPDATE `creature` SET `position_x` = 3020.17, `position_y` = -3177.85, `position_z` = 294.146, `orientation` = 6.05629, `spawntimesecs` = 3600, `wander_distance` = 8 WHERE `guid` = 128068;
UPDATE `creature` SET `position_x` = 3009.26, `position_y` = -3178.27, `position_z` = 294.146, `orientation` = 3.05433, `spawntimesecs` = 3600, `wander_distance` = 8 WHERE `guid` = 128069;
UPDATE `creature` SET `position_x` = 3006.07, `position_y` = -3250.05, `position_z` = 294.146, `orientation` = 3.03687, `spawntimesecs` = 3600 WHERE `guid` = 128096;
UPDATE `creature` SET `position_x` = 3005.44, `position_y` = -3248.76, `position_z` = 294.146, `orientation` = 2.89725, `spawntimesecs` = 3600 WHERE `guid` = 128085;
UPDATE `creature` SET `position_x` = 3000.56, `position_y` = -3248.15, `orientation` = 0.349066, `spawntimesecs` = 3600 WHERE `guid` = 128091;
UPDATE `creature` SET `position_x` = 3004.78, `position_y` = -3246.98, `orientation` = 4.11898, `spawntimesecs` = 3600 WHERE `guid` = 128094;
UPDATE `creature` SET `position_x` = 3003.09, `position_y` = -3245.75, `orientation` = 3.87463, `spawntimesecs` = 3600 WHERE `guid` = 128086;
UPDATE `creature` SET `position_x` = 3001.31, `position_y` = -3244.87, `orientation` = 4.74729, `spawntimesecs` = 3600 WHERE `guid` = 128095;
UPDATE `creature` SET `position_x` = 3025.94, `position_y` = -3139.97, `position_z` = 294.163, `orientation` = 5.35816, `spawntimesecs` = 3600 WHERE `guid` = 128092;
UPDATE `creature` SET `position_x` = 3026.32, `position_y` = -3146.59, `position_z` = 294.161, `orientation` = 1.81514, `spawntimesecs` = 3600 WHERE `guid` = 128098;
UPDATE `creature` SET `position_x` = 3028.9, `position_y` = -3145.06, `position_z` = 294.16, `orientation` = 2.37365, `spawntimesecs` = 3600 WHERE `guid` = 128088;
UPDATE `creature` SET `position_x` = 3030.39, `position_y` = -3142.86, `position_z` = 294.16, `orientation` = 2.56563, `spawntimesecs` = 3600 WHERE `guid` = 128097;
UPDATE `creature` SET `position_x` = 3031.03, `position_y` = -3140.48, `position_z` = 294.16, `orientation` = 2.82743, `spawntimesecs` = 3600 WHERE `guid` = 128087;
UPDATE `creature` SET `position_x` = 3031.18, `position_y` = -3137.42, `position_z` = 294.161, `orientation` = 3.36848, `spawntimesecs` = 3600 WHERE `guid` = 128099;
DELETE FROM `creature` WHERE `id1` IN (1748, 1749);
DELETE FROM `creature` WHERE `guid` BETWEEN @creatureId+9 AND @creatureId+32;
