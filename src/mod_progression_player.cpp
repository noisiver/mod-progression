...
{
    ChatHandler(player->GetSession()).SendSysMessage(PatchNotes[patchId][1]);
...
{
    if ((factionID == FACTION_SILVERMOON_CITY || factionID == FACTION_EXODAR) && patchId < BEFORE_THE_STORM)
    {
        return false;
    }
...
{
    if (patchId < THE_GODS_OF_ZUL_AMAN && quest->GetQuestLevel() >= 30 && quest->GetQuestLevel() <= 60)
    {
...
{
    if (xpSource == PlayerXPSource::XPSOURCE_BATTLEGROUND && patchId < CALL_OF_THE_CRUSADE)
    {
        amount = 0;
    }
}
