...
DELETE FROM `npc_vendor` WHERE `entry` IN (228, 734, 789, 1149, 1285, 1298, 1462, 2084, 2401, 2803, 2806, 2808, 2820, 2839, 2908, 3015, 3313, 3350, 3410, 3541, 4170, 4173, 4241, 4555, 4604, 4892, 4896, 5101, 5122, 7942, 8139, 8362, 9548, 9551, 9552, 9555, 11038, 11555, 12021, 12027, 12959, 12960, 14301, 14624, 15174, 17598, 20080) AND `item` = 28053;
...
DELETE FROM `npc_vendor` WHERE `entry` IN (1325, 1326, 1457, 2622, 3090, 3135, 3334, 3490, 3542, 3551, 3561, 3969, 4585, 5169, 6779, 15175) AND `item` IN (22053, 22055, 21927, 21835, 22054);
DELETE FROM `npc_vendor` WHERE `entry` = 789 AND `item` = 11285;
DELETE FROM `npc_vendor` WHERE `entry` IN (844, 958, 983, 989, 1148, 1257, 1286, 1313, 1453, 2380, 2480, 2481, 2810, 2812, 2821, 2848, 3010, 3014, 3348, 3367, 3405, 3548, 3779, 3954, 3955, 3956, 4216, 4226, 4229, 4561, 4610, 4615, 4775, 4897, 4899, 5138, 5163, 5178, 5503, 5594, 5817, 6301, 7947, 8145, 8157, 8158, 8177, 8178, 8305, 8363, 11188, 11189, 12022, 12028, 12245, 12941, 12957, 12958, 15179) AND `item` = 18256;
DELETE FROM `npc_vendor` WHERE `entry` IN (734, 1149, 1285, 2084, 2401, 2685, 2803, 2806, 2808, 2820, 2908, 3313, 3322, 3350, 3541, 4170, 4241, 4555, 4896, 5101, 7942, 8131, 8139, 8362, 9548, 11038, 11184, 11555, 12021, 12027, 12959, 12960, 14624, 15174) AND `item` = 28060;
DELETE FROM `npc_vendor` WHERE `entry` = 6028 AND `item` IN (2102, 2512, 2515, 2516, 2519, 3030, 3033, 5439, 11284, 11285, 11363, 28053, 28060);
...
(6028, 2102, 0, 0, 0, 0), -- Small Ammo Pouch
(6028, 2512, 0, 0, 0, 0), -- Rough Arrow
(6028, 2515, 0, 0, 0, 0), -- Sharp Arrow
(6028, 2516, 0, 0, 0, 0), -- Light Shot
(6028, 2519, 0, 0, 0, 0), -- Heavy Shot
(6028, 3030, 0, 0, 0, 0), -- Razor Arrow
(6028, 3033, 0, 0, 0, 0), -- Solid Shot
(6028, 5439, 0, 0, 0, 0), -- Small Quiver
(6028, 11284, 0, 0, 0, 0), -- Accurate Slugs
(6028, 11285, 0, 0, 0, 0), -- Jagged Arrow
(6028, 11363, 0, 0, 0, 0), -- Medium Shot Pouch
...
-- Vendor-Tron 1000
(12245, 18256, 0, 0, 0, 0), -- Imbued Vial
